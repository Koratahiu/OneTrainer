...
class DoRAOFTModule(OFTModule):
    ...
    def initialize_weights(self):
        ...
        if isinstance(self.orig_module, nn.Linear):
            ...
            # Norm across input dimension (dim 1) -> Shape: [out_features, 1]
            norm = torch.norm(weight, dim=1, keepdim=True)
        elif isinstance(self.orig_module, nn.Conv2d):
            weight = self.orig_module.weight.detach().float()
            # Norm across [in_channels, k, k] -> Shape: [out_channels, 1, 1, 1]
    ...
    def forward(self, x, *args, **kwargs):
        # Get the standard OFT output (which includes the rotation)
        # result = W_orig @ R^T @ x + bias
        result = super().forward(x, *args, **kwargs)

        # Remove the original bias temporarily (we want to scale the weights, not the bias)
        # OFTModule.forward applies the bias at the end.
        bias = self.orig_module.bias
        if bias is not None:
            # Reshape bias for broadcasting if necessary
        ...
        # Apply DoRA Scaling
        # W_final = (m / ||W_orig||) * (W_orig @ R^T)
        # Output = W_final @ x = (m / ||W_orig||) * result_without_bias

        # We add epsilon to denominator for numerical stability, though norms shouldn't be 0
        scale = (self.dora_scale) / (self.initial_norm + 0)

        if isinstance(self.orig_module, nn.Linear):
            # Target: [1, Out] to broadcast against [Batch, ..., Out]
            scale = scale.view(1, -1)
        elif isinstance(self.orig_module, nn.Conv2d):
            # Target: [1, Out, 1, 1] to broadcast against [Batch, Out, H, W]
