...
class OFTInspectorApp:
    ...
    def get_metrics(self, path):
        ...
        try:
            ...
            # --- Pre-scan for DoRA Scale ---
            dora_scales = {}
            for k, v in sd.items():
                if 'dora_scale' in k:
                    base = k.replace('.dora_scale', '')
                    dora_scales[base] = v.float()
            # -------------------------------

            for k, v in sd.items():
                if 'oft' not in k.lower() and 'weight' not in k: continue
                if 'dora_scale' in k: continue 
                ...
                # --- DoRA Norm Checking ---
                has_dora = base in dora_scales
                dora_norm = torch.linalg.norm(dora_scales[base]).item() if has_dora else 0.0
    ...
    def format_tree_columns(self, cols, headers):
        ...
        for col, head in zip(cols, headers):
            ...
            width = 300 if col == "layer" else 110
    ...
    def run_analysis(self):
        ...
        if mode == "single":
            cols = ("layer", "blocks", "block_size", "w_l2", "rot_shift", "has_dora", "dora_norm")
            headers = ["Layer Name", "Blocks (R)", "Block Size", "Weight (L2 Norm)", "Rotation Shift (‖R-I‖)", "DoRA", "DoRA Norm"]
            self.format_tree_columns(cols, headers)

            for k in sorted(r1.keys()):
                d = r1[k]
                self.results_data.append({"layer": k, **d})
                self.tree.insert("", "end", values=(k, d['blocks'], d['block_size'], f"{d['w_l2']:.4f}", f"{d['rot_shift']:.4f}", "Yes" if d['has_dora'] else "No", f"{d['dora_norm']:.4f}"))
            ...
            headers = ["Layer Name", "OFT1 W(L2)", "OFT2 W(L2)", "Δ W(L2)%", "OFT1 RotShift", "OFT2 RotShift", "Δ RotShift%", "DoRA(1|2)", "Δ DoRA Norm%"]
            self.format_tree_columns(cols, headers)
            ...
            for k in all_keys:
                d1 = r1.get(k, {'w_l2':0, 'rot_shift':0, 'has_dora':False, 'dora_norm':0.0})
                d2 = r2.get(k, {'w_l2':0, 'rot_shift':0, 'has_dora':False, 'dora_norm':0.0})

                diff_w = self.calc_pct_diff(d1['w_l2'], d2['w_l2'])
                diff_rot = self.calc_pct_diff(d1['rot_shift'], d2['rot_shift'])
                diff_dora = self.calc_pct_diff(d1['dora_norm'], d2['dora_norm'])
                ...
                row_data = {
                    ...
                    "dora_1": d1['dora_norm'], "dora_2": d2['dora_norm'], "diff_dora": diff_dora,
                ...
                g["dora_1"] += d1['dora_norm']; g["dora_2"] += d2['dora_norm']
            ...
            if gd_dora > 0 or g["dora_1"] > 0 or g["dora_2"] > 0:
                self.global_diff_var.set(f"Global Drift → W(L2): {gd_w:+.2f}%  |  Rot: {gd_r:+.2f}%  |  DoRA: {gd_dora:+.2f}%")
            else:
    ...
    def show_charts(self):
        ...
        if self.mode_var.get() == "single":
            ...
            # Tab 3: DoRA Norm (Conditional)
            if any(r.get("has_dora") for r in self.results_data):
                f3 = plt.Figure(figsize=(10, 5), facecolor=self.bg_color)
                ax3 = f3.add_subplot(111)
                ax3.bar(x, [r["dora_norm"] for r in self.results_data], 0.5, label='DoRA Scale Norm', color='#ab47bc')
                ax3.set_title("DoRA Scale Magnitude per Layer")
                ax3.set_xticks(x); ax3.set_xticklabels(layers, rotation=90, fontsize=8)
                ax3.legend()
                self.add_tab(notebook, f3, "DoRA Norm")

        else:
            has_dora = any(r.get("dora_1", 0) > 0 or r.get("dora_2", 0) > 0 for r in self.results_data)
            ...
            if has_dora:
                ...
                ax_dora.plot(x, [r["dora_1"] for r in self.results_data], label='OFT 1 - DoRA Norm', alpha=0.8, color='#ab47bc')
                ax_dora.plot(x, [r["dora_2"] for r in self.results_data], label='OFT 2 - DoRA Norm', alpha=0.8, color='#ff7043')
                ax_dora.fill_between(x, [r["dora_1"] for r in self.results_data], [r["dora_2"] for r in self.results_data], color='gray', alpha=0.2)
                ax_dora.set_title("DoRA Scale Magnitude Comparison")
            ...
            if has_dora:
                ax3.scatter(x, [r["diff_dora"] for r in self.results_data], label='Δ DoRA Norm %', color='#ffca28', marker='^')
