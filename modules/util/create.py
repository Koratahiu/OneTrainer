...
def create_optimizer(
    ...
    match config.optimizer.optimizer:
        ...
        case Optimizer.MLorc_Prodigy:
            from MLorc_optim import MLorc_Prodigy
            optimizer = MLorc_Prodigy(
                ...
                rank=optimizer_config.rank if optimizer_config.rank is not None else 8,
                ...
                growth_rate=optimizer_config.growth_rate if optimizer_config.growth_rate is not None else float('inf'),
                slice_p=optimizer_config.slice_p if optimizer_config.slice_p is not None else 1,
                disable_mlorc=optimizer_config.use_grams if optimizer_config.disable_mlorc is not None else False,
            )

        # MLorc_DAdapt_Lion Optimizer
        case Optimizer.MLorc_DAdapt_Lion:
            from MLorc_optim import MLorc_DAdapt_Lion
